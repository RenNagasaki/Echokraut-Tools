...
namespace PrepareAlltalkTrainingData
{
    public static class GetScdHelper
    {
        ...
        public static void WorkCutScenes(ARealmReversed realm, Language language, string savePath)
        {
                ...
                foreach (var audioLine in fileData)
                {
                    ...
                    if (audioLine.ToLower().Contains("voiceman") || audioLine.ToLower().Contains("manfst"))
                    {
                        ...
                        if (audioFileSplit.Length == 5)
                        {
                            ...
                            var wantedLines = WorkText(text, language);

                            if (wantedLines == null || String.IsNullOrWhiteSpace(wantedLines[0].Trim()))
                                continue;

                            savePathFolder = savePath + @"\temp\" + (character.Trim().Length > 0 ? character : "system");
                            AddLine(realm, languageString, savePathFolder, wantedLines[0], audioFile + "m_", character, characterLines);
                            if (wantedLines.Length > 1)
                                AddLine(realm, languageString, savePathFolder, wantedLines[1], audioFile + "f_", character, characterLines);
                        }
                    }
            ...
            foreach (var character in characterLines.Keys)
            {
                PrepareBatch(savePath + @"\temp\" + character);
            }

            ConvertFiles(savePath + @"\temp\");
            System.IO.Directory.Delete(savePath + @"\temp\", true);
        ...
        private static void AddLine(ARealmReversed realm, string languageString, string savePathFolder, string line, string audioFile, string character, Dictionary<string, List<string>> characterLines)
        {
            var localText = line;
            ...
            foreach (var expansion in PackIdentifier.ExpansionToKeyMap.Keys)
            {
                ...
                var result = ExportScdFile(realm, audioPathFull, savePathFolder);

                if (result)
                {
                    if (!characterLines.ContainsKey(character))
                        characterLines.Add(character, new List<string>());
                    characterLines[character].Add("wavs\\" + (character.Trim().Length > 0 ? character : "system") + @"\" + fileName + "|" + localText + "|" + character);
                    break;
                }
            }
        }

        private static bool ExportScdFile(ARealmReversed realm, string filePath, string savePathFolder)
        {
            try
            {
                ...
                for (var i = 0; i < scdFile.ScdHeader.EntryCount; ++i)
                {
                    ...
                    var targetPath = System.IO.Path.Combine(savePathFolder, fileNameWithoutExtension);
                    ...
                    var fInfoTemp = new System.IO.FileInfo(targetPath);
                    var fInfo = new System.IO.FileInfo(targetPath.Replace("temp", "wavs"));

                    //if (!fInfoRaw.Directory.Exists) 
                    //    fInfoRaw.Directory.Create();
                    if (!fInfoTemp.Directory.Exists)
                        fInfoTemp.Directory.Create();
                    if (!fInfo.Directory.Exists)
                        fInfo.Directory.Create();
                    //System.IO.File.WriteAllBytes(fInfoTemp.FullName, e.File.SourceFile.GetData());
                    System.IO.File.WriteAllBytes(fInfoTemp.FullName, e.GetDecoded());
                }
        ...
        public static void PrepareBatch(string inPath)
        {
            if (System.IO.Directory.Exists(inPath))
            {
                foreach (var file in System.IO.Directory.GetFiles(inPath))
                {
                    batch.Add($@"ffmpeg\ffmpeg.exe -n -codec:a adpcm_ms -i {file} -f wav {file.Replace("temp", "wavs")}");
                }
            }
        }

        public static void ConvertFiles(string saveFolderPath)
        {
            System.IO.File.WriteAllLines(saveFolderPath + @"\convert.bat", batch);
            ProcessStartInfo cmdsi = new ProcessStartInfo(saveFolderPath + @"\convert.bat");
        ...
        private static string[] WorkText(string text, Language language)
        {
            //LogHelper.logData($"Working on Text: {text}");
            text = text.Substring(1, text.Length - 2).Trim();
            text = CleanUpLine(text, language);
        ...
        private static string CleanUpLine(string line, Language language)
        {
            line = line
                .Replace("<br>", " ")
                .Replace("<i>", "")
